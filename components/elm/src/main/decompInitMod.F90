module decompInitMod
  ...
  public decompInit_user_defined_decomposition    ! Implementation of user defined decomposition
...
contains
  ...
  !------------------------------------------------------------------------------
  subroutine decompInit_user_defined_decomposition(lni,lnj,amask)
    !
    ! !DESCRIPTION:
    ! This subroutine initializes the land surface decomposition into a clump
    ! data structure.  This assumes each pe has the same number of clumps
    ...
    do ln = 1,lns
       if (amask(ln) > 0) then
          ...
          !--- amask represents the mask values defined in the domain file.  
          !--- In user-defined mode, each amask value indicates which clump a grid cell belongs to.
          cid = amask(ln)
          lcid(ln) = cid
    ...
    if (masterproc) then
       write(iulog,*)' Load user defined decomposition successfully'
       write(iulog,*)' Surface Grid Characteristics'
       write(iulog,*)'   longitude points               = ',lni
       write(iulog,*)'   latitude points                = ',lnj
       write(iulog,*)'   total number of land gridcells = ',numg
  ...
  end subroutine decompInit_user_defined_decomposition
